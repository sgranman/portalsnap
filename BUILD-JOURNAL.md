# PortalSnap build journal

This is the web app's build journal, moved here from the README so that page can stay short. It
runs roughly in the order things happened: what the Portal's browser turned out to be capable
of, and the decisions each measurement forced. It is long because the measurements are the
point — nearly every design choice here was made by a number rather than a preference. The native
Android app keeps its own notes in [android/README.md](android/README.md) and
[android/PHOTOBOOTH.md](android/PHOTOBOOTH.md).
...
These are how nearly every number in this journal was arrived at, and they stay in the repo
...
— the probe table at the top of this journal says so — and never loads it, but a device
